...
export default function PocketPage() {
    ...
    async function fetchRewards() {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('action_key, label, description, icon, amount, daily_cap, sort_order')
          .order('sort_order');
        if (cancelled) return;
        if (error) {
          setRewardsError('Could not load earning opportunities.');
        } else {
          setRewards(data ?? []);
        }
      } catch {
        if (!cancelled) setRewardsError('Could not load earning opportunities.');
      } finally {
  ...
  return (
    <div className="space-y-12 max-w-2xl mx-auto pb-40 px-4 pt-12">
      ...
      {/* ── 5. Earning Rites (DB-backed reward config) ─── */}
      <section className="space-y-6">
        <div className="px-2">
        ...
        {!rewardsLoading && rewardsError && (
          <p className="text-center text-xs text-rose-300/70 py-4">{rewardsError}</p>
        )}
            ...
            <AnimatePresence>
              {rewards.map((item) => (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="min-w-[140px] p-5 rounded-3xl bg-noctvm-surface/60 border border-white/5 shadow-xl flex flex-col items-center text-center"
                >
                  ...
                  <p className="text-[9px] text-noctvm-silver/40 leading-tight mb-4">{item.description}</p>
