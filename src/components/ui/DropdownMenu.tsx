...
function DropdownMenuRadioItem({ className, children, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.RadioItem>) {
  return (
    ...
    >
      <span className="pointer-events-none absolute left-2 flex size-3.5 items-center justify-center">
        <DropdownMenuPrimitive.ItemIndicator>
          <CircleIcon className="size-2 fill-noctvm-violet text-noctvm-violet" />
        </DropdownMenuPrimitive.ItemIndicator>
      </span>
      {children}
    </DropdownMenuPrimitive.RadioItem>
...
function DropdownMenuLabel({ className, inset, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Label> & { inset?: boolean }) {
  return (
    <DropdownMenuPrimitive.Label
      ...
      data-inset={inset}
      className={cn('px-2 py-1.5 text-xs font-medium text-noctvm-silver data-[inset]:pl-8', className)}
      {...props}
    />
  );
}
...
function DropdownMenuSeparator({ className, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Separator>) {
  return (
    <DropdownMenuPrimitive.Separator
      ...
      className={cn('-mx-1 my-1 h-px bg-noctvm-border', className)}
      {...props}
    />
  );
}
...
function DropdownMenuShortcut({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span
      ...
      className={cn('ml-auto text-xs tracking-widest text-noctvm-silver', className)}
      {...props}
    />
  );
}
...
function DropdownMenuSubTrigger({ className, inset, children, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.SubTrigger> & { inset?: boolean }) {
  return (
    ...
    >
      {children}
      <ChevronRightIcon className="ml-auto size-4 text-noctvm-silver" />
    </DropdownMenuPrimitive.SubTrigger>
  );
}
