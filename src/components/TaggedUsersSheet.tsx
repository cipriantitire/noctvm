...
import CurvedScrollBar from '@/components/ui/CurvedScrollBar';
...
export default function TaggedUsersSheet({ handles, isOpen, onClose }: TaggedUsersSheetProps) {
  const { user } = useAuth();
  ...
  const contentHeight = Math.min(users.length, MAX_VISIBLE_ROWS) * ROW_HEIGHT;
  const needsScroll = users.length > MAX_VISIBLE_ROWS;

  const UserList = () => (
    <div className="divide-y divide-noctvm-border/50">
      {users.map((u) => (
        <div key={u.id} className="group flex items-center justify-between gap-3 p-3" style={{ height: ROW_HEIGHT }}>
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <SheetContent 
        side="bottom" 
        className="bg-noctvm-black border-noctvm-border border-t p-0 rounded-t-2xl min-h-0 flex flex-col corner-smooth"
        style={{ height: 'auto', maxHeight: `${(MAX_VISIBLE_ROWS * ROW_HEIGHT) + 56}px` }}
      >
        <SheetHeader className="p-4 border-b border-noctvm-border flex-shrink-0">
          <SheetTitle className="font-heading font-bold text-base">Tagged People</SheetTitle>
        </SheetHeader>
        
        {loading ? (
          <div className="p-8 text-center" style={{ height: ROW_HEIGHT * 3 }}>
          ...
          <CurvedScrollBar 
            viewportClassName="overflow-y-auto scrollbar-hide"
            cornerRadius={12}
            showRail
