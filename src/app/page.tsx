...
export default function Home() {
  ...
  if (selectedVenue) {
    return (
      <div className="flex min-h-screen bg-noctvm-black overflow-x-hidden">
        <Sidebar activeTab={activeTab} onTabChange={(t) => { setActiveTab(t); setSelectedVenue(null); }} />
        <main className="flex-1 min-w-0 min-h-screen">
          ...
          <VenuePage venueName={selectedVenue} onBack={() => setSelectedVenue(null)} />
        </main>
        <BottomNav activeTab={activeTab} onTabChange={(t) => { setActiveTab(t); setSelectedVenue(null); }} />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-noctvm-black overflow-x-hidden">
            ...
            <div className="space-y-6 max-w-lg mx-auto">
              ...
              <div className="space-y-2">
                {[
                  ...
                  >
                    <div className="w-9 h-9 rounded-lg bg-noctvm-midnight flex items-center justify-center text-noctvm-silver group-hover:text-noctvm-violet transition-colors flex-shrink-0">
                      {item.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white">{item.label}</p>
                      <p className="text-[10px] text-noctvm-silver mt-0.5">{item.desc}</p>
                    </div>
                    <svg className="w-4 h-4 text-noctvm-silver group-hover:text-noctvm-violet transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                  </button>
                ))}
              </div>
            </div>
      ...
      {activeTab === 'events' && <RightPanel onVenueClick={handleVenueClick} />}
