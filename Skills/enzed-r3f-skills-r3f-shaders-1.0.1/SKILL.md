---
name: r3f-shaders
description: React Three Fiber shaders - GLSL, shaderMaterial, uniforms, custom effects. Actions: create, modify, optimize, review shaders.

## 🧬 Evolution & Self-Improvement

- **Observe**: Check if HMR (Hot Module Replacement) is breaking during shader edits.
- **Inspect**: If the scene doesn't update, inspect if the `key={Material.key}` is missing or if uniforms are not reactive.
- **Amend**: Add new reusable GLSL noise or lighting snippets to the "Common Shader Patterns" section as they are developed.
---
