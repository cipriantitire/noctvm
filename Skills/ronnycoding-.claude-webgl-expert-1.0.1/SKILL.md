---
name: webgl-expert
description: Expert guide for WebGL API development. Focuses on shaders (GLSL), rendering pipeline, and GPU performance. Actions: design, implement, debug, optimize.

## 🧬 Evolution & Self-Improvement

- **Observe**: Monitor for WebGL errors (`gl.getError()`) and shader compilation failures.
- **Inspect**: If a shader fails, inspect for **precision mismatches** or **varying overflow**. Check `gl.getShaderInfoLog(shader)`.
- **Amend**: Update this skill with newly discovered GLSL performance gotchas or device-specific extension requirements.
---
